...
@Component({
  selector: 'app-dropoff-review-page',
  imports: [RouterLink, ContributionPanelComponent],
  templateUrl: './dropoff-review-page.component.html',
  styleUrl: './dropoff-review-page.component.scss'
})
export class DropoffReviewPageComponent {
  ...
  protected draft: DropoffFlowDraft | null = null;
  ...
  constructor() {
    this.draft = this.state.getDropoffDraft();
  }
  ...
  protected updateContributionAmount(amount?: number): void {
    ...
    this.state.setDropoffDraft(this.draft);
  }
  ...
  protected async launchContribution(amount?: number): Promise<void> {
    ...
    try {
      ...
      this.contributionError =
        'We could not open contribution checkout right now. You can still submit your drop-off request.';
    } finally {
  ...
  protected async submitRequest(): Promise<void> {
    ...
    const payload: CreateDonationRequestPayload = {
      ...
      dropoff: {
        ...
        dropoffNotes: this.draft.dropoffNotes,
        locationName: this.warehouse.name,
        locationAddress: this.warehouse.address
      },
      metadata: {
        flowVersion: 'mvp-v1',
        channel: 'public-web'
    ...
    try {
      ...
      this.state.setDropoffConfirmation(result);
      this.state.clearDropoffDraft();
      await this.router.navigate(['/dropoff/confirmation']);
    } catch {
